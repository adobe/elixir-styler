...
defmodule Styler.Style.DeprecationsTest do
  ...
  describe "1.17+" do
    ...
    test "DateTime.add/2,3,4 => DateTime.shift/2" do
      for unit <- ~w(day hour minute second), n <- ["-1", "1", "5", "-5", "0", "var", "-var", "5 + x"] do
    ...
    test "rewritten deprecations get picked up by pipes " do
      assert_style """
                   filter.value
