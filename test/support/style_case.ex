  ...
  defmacro assert_style(before, expected \\ nil) do
    ...
    quote bind_quoted: [before: before, expected: expected], location: :keep do
      ...
      if styled != expected and ExUnit.configuration()[:trace] do
        ...
        dbg(before_ast, limit: :infinity)
        dbg(before_comments, limit: :infinity)
        IO.puts("======Expected AST==========\n")
        {expected_ast, expected_comments} = Styler.string_to_ast(expected)
        dbg(expected_ast, limit: :infinity)
        dbg(expected_comments, limit: :infinity)
        IO.puts("======Got AST===============\n")
        dbg(styled_ast, limit: :infinity)
        dbg(styled_comments, limit: :infinity)
        IO.puts("========================\n")
      end
